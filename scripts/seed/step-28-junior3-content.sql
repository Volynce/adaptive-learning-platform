-- Step-27: Junior_2 content seed (rank=junior, level=3)
...
WHERE NOT EXISTS (
  SELECT 1
  FROM articles a
  WHERE a.content_ref = ('local://articles/' || m.name || '/junior-3/intro')
);
...
SELECT
  ...
  ('local://articles/' || m.name || '/junior-3/practice'),
  'junior',
  3,
  true
FROM modules m
WHERE NOT EXISTS (
  SELECT 1
  FROM articles a
  WHERE a.content_ref = ('local://articles/' || m.name || '/junior-3/practice')
);

-- 3) MINITEST (3 вопроса) для practice-статей junior_3
...
--   LIMIT 3
-- ) q ON true
-- WHERE a.target_rank='junior' AND a.target_level=3
--   AND a.content_ref LIKE 'local://articles/%/junior-3/practice'
--   AND NOT EXISTS (
